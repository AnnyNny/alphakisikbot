...
quest_steps = [
    ...
    },
    {
        "text": """ВЖУХ! ВЖУХ! ВЖУХ! Цифры высокоранговой матрицы начинают выбегать по ткани пространства до тех пор, пока стрим байтов не заканчивает передачу и не конвертируется в объемную версию Ани Сметаниной.\n"
                Она недовольно окидывает тебя взглядом с ног до головы и поджимает губы. Кажется, у нее нет никаких воспоминаний о встрече с тобой. "Ну что там ещё? Быстро говори, у меня нет на тебя времени"\n"""
    ...
    {
        "text": "Ладно, шутка! Сапиосексуальность оставим для просмотра Вы легли на кровать посмотреть фильм. Наступает момент тишины. Что будешь делать?\n"
                "A) [Харизма] Сделать комплимент\n"
                "Б) [Альфа] Признаться, что у тебя нет пижамы \n"
                "В) Молчать",
        "choices": [
            ("A", "dice_roll_charisma"),
            ("Б", "dice_roll_alpha"),
    ...
    {
        "text": "Она предлагает поделиться своими целями в жизни. Как ответите?\n"
                "A) Рассказать о своей мечте\n"
                "Б) Пошутить на тему целей\n"
                "В) Сказать, что не думаете о будущем",
        "choices": [
            ("A", 2),
            ("Б", 1),
            ("В", -1)
    ...
    {
        "text": "Вы подошли к завершению фильма. Чем займемся?\n"
                "A) [Альфа] Сделать неприличный подкат\n"
                "Б) Чмокнуть её в носик\n"
                "В) Пойду за сырками в Пятёрочку",
        "choices": [
            ("A", "dice_roll_alpha"),
            ("Б", 1),
            ("В", 1)
...
async def choose_skill(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await query.message.reply_text(f"Ты получаешь: {skill.capitalize()}. Возможно он тебе пригодится...")
    user_data[user_id]["messages"].append(msg.message_id)
...
async def ask_step(update_or_query, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Воспроизведение музыки перед последним шагом
    if step_index == 8:
        final_music_link = "https://open.spotify.com/track/final_music"
        msg = await update_or_query.message.reply_text(f"Финальная музыка: [Послушать здесь]({final_music_link})", parse_mode="Markdown")
        user_data[user_id]["messages"].append(msg.message_id)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if isinstance(choice_data, str) and "dice_roll" in choice_data:
        ...
        if total_roll >= 15:
            ...
            msg = await query.message.reply_text("Критический успех! Вы великолепны.")
        elif total_roll >= 10:
            user_data[user_id]["points"] += 1
            msg = await query.message.reply_text("Успешно, но не 12 из 10.")
        else:
            msg = await query.message.reply_text("Неудачно... Видимо, вам не повезло, в следующий раз повезет больше.")
...
async def calculate_ending(update_or_query, context: ContextTypes.DEFAULT_TYPE):
    ...
    if points >= 6:
        ending = "Поздравляем! У вас прекрасное свидание, и всё заканчивается счастливо!"
        photo_path = "happy_ending.png"  # Картинка для хорошей концовки
    elif points >= 3:
        ending = "Свидание прошло нормально. Возможно, стоит попробовать ещё раз."
        photo_path = "neutral_ending.png"  # Картинка для нейтральной концовки
    else:
        ending = "Увы, свидание провалилось. Она больше не хочет с вами общаться."
    ...
    # Таймер на 10 секунд для удаления изображения
